...
public class BlockInfo extends Block implements LightWeightGSet.LinkedElement {
	...
	public int compareTo(Block b) {
		...
		KthFsHelper.printKTH("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ BlockInfo.comapreTo called for blkid: " + binfo.getBlockId() 
							 + " blockIndex:"+binfo.getBlockIndex());
		if(binfo.getBlockIndex() == -1)
			return -1;
		else
	...
	public int compareTo(Block b) {
		...
		if (b instanceof BlockInfo) {
			...
			Exception exe = new Exception("compareTo custom exception BlockInfo");
			try {
				throw exe;
			} catch(Exception e) {
				e.printStackTrace();
			...
			Exception exe = new Exception("compareTo custom exception Block");
			try {
				throw exe;
			} catch(Exception e) {
				e.printStackTrace();
